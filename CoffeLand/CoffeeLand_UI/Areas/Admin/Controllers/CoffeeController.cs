        ...
        public ActionResult Create([Bind(Include = "ID,CoffeeName,Description,Price,AVGPoint,ImageUrl,AltText,IsPrepared,ExtraMaterialsID,CategoryID")] Coffee coffee, HttpPostedFileBase file)
        {
            ...
            else if (customer.AuthorizationID == 1 || customer.AuthorizationID == 2)
            {
                if (ModelState.IsValid)
                {
                    if (file != null)
                    {
                        file.SaveAs(HttpContext.Server.MapPath("~/images/urunler/") + file.FileName);
                        string path = HttpContext.Server.MapPath("~/images/urunler/") + file.FileName;

                        int baslangic = path.IndexOf(@"\images");
                        int uzunluk = path.Length - baslangic; 
                        path = path.Substring(baslangic, uzunluk);

                        coffee.ImageUrl = path;
                    }
